...
void MainWindow::initUserScript()
{
    ...
    if (CommandParameters::hasScript()) {
        QString file = CommandParameters::args->value("script");
        QFileInfo fileInfo(file);
        // 尝试直接路径探测文件存在否
        if (!fileInfo.exists()) {
            file = getAppPath() + CommandParameters::args->value("script");
            fileInfo.setFile(file);
            // 尝试从应用程序路径探测文件存在否
            if (!fileInfo.exists()) {
                printf("%s", (file + " File not found.").toLatin1().data());
                QApplication::quit();
                return;
            }
        }
...
QScriptValue MainWindow::print(QScriptContext *context, QScriptEngine *interpreter)
{
    ...
    output += "\n";
    printf("%s", output.toLocal8Bit().data());
    return QScriptValue::UndefinedValue;
}
...
void MainWindow::onTimeout()
{
    QScriptEngine* interpreter = script->getScriptEngine();
    if (interpreter->hasUncaughtException()) {
        printf(getScriptError(interpreter).toLocal8Bit().data());
        QApplication::quit();
    }
}
